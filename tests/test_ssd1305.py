...
def test_init_128x32():
    """
    SSD1305 OLED with a 128 x 32 resolution works correctly.
    """
    ssd1305(serial)
    serial.command.assert_has_calls([
        ...
        call(ssd1306_const.DISPLAYOFF, ssd1306_const.SETDISPLAYCLOCKDIV, 128,
             ...
             ssd1306_const.SETSTARTLINE, ssd1306_const.CHARGEPUMP, 16,
        ...
        # reset the display (note colstart=4, colend=131)
        call(ssd1306_const.COLUMNADDR, 4, 131, ssd1306_const.PAGEADDR, 0, 3),
...
def test_init_128x64():
    """
    SSD1305 OLED with a 128 x 64 resolution works correctly.
    """
    ssd1305(serial, width=128, height=64)
    serial.command.assert_has_calls([
        call(ssd1306_const.DISPLAYOFF, ssd1306_const.SETDISPLAYCLOCKDIV, 128,
...
def test_init_invalid_dimensions():
    """
    SSD1305 OLED with an invalid resolution raises a
    :py:class:`luma.core.error.DeviceDisplayModeError`.
    """
    assert_invalid_dimensions(ssd1305, serial, 59, 22)


def test_hide():
    """
    SSD1305 OLED screen content can be hidden.
    """
    device = ssd1305(serial)
    serial.reset_mock()
    device.hide()
...
def test_show():
    """
    SSD1305 OLED screen content can be displayed.
    """
    device = ssd1305(serial)
    serial.reset_mock()
    device.show()
...
def test_display():
    """
    SSD1305 OLED screen can draw and display an image.
    """
    device = ssd1305(serial)
    serial.reset_mock()
    ...
    serial.command.assert_called_once_with(ssd1306_const.COLUMNADDR, 4, 131, ssd1306_const.PAGEADDR, 0, 3)
    serial.data.assert_called_once_with(get_reference_data('demo_ssd1305'))
